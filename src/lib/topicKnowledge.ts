...
export const topicKnowledge = {
  ...
  agentArchitecture: {
    what: "Agent Architecture defines how autonomous AI agents are built with four core components: Perception (processing inputs via text, tools, or multimodal), Reasoning (planning strategies like ReAct or Plan-and-Execute), Memory (retaining context from simple buffers to vector stores), and Action (executing through tool-calling or code). Unlike LLMs that just predict text or workflows that follow fixed sequences, agents pursue goals autonomously with explicit reasoning and adaptive behavior.",
    why: "Agent Architecture is essential for building truly autonomous AI systems that can handle complex, multi-step tasks requiring dynamic replanning and learning from failures. Proper architecture enables agents to maintain context across sessions, recover from errors through reflection, scale to enterprise use cases with persistent memory, and collaborate with humans by explaining their reasoning process.",
    how: "Build agent architectures by: 1) Designing the perception system (text-based for chat, tool-augmented for APIs/databases, multimodal for GUI automation), 2) Selecting a reasoning framework (ReAct for dynamic tasks, Plan-and-Execute for complex workflows, DPPM for parallel execution), 3) Implementing memory (start with context-window, progress to vector stores like Zep, scale to memory stacks like Mem0), 4) Configuring action systems with structured tool-calling and dynamic parameters."
  } as TopicKnowledge
};